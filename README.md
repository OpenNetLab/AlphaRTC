...
- Dockerized environment
    ...
    PyInfer:
    ```shell
    sudo docker run -d --rm -v `pwd`/examples/peerconnection/serverless/corpus:/app -w /app --name alphartc alphartc peerconnection_serverless_pyinfer receiver_cmdinfer.json
    sudo docker exec alphartc peerconnection_serverless sender.json
    ```

    ONNXInfer:
    ``` shell
    sudo docker run -d --rm -v `pwd`/examples/peerconnection/serverless/corpus:/app -w /app --name alphartc alphartc peerconnection_serverless_pyinfer sender_cmdinfer.json
    sudo docker exec alphartc peerconnection_serverless sender.json
    ```
